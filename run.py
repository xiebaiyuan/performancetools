...
def save_all_op_output(feed_kv=None):
    ...
    if not os.path.exists(output_mobile_path):
        os.mkdir(output_mobile_path)
    ops = prog.current_block().ops
    fetch_names = []
...
def check_mobile_results(args, fuse, mem_opt):
    ...
    #         print(line)
    optimization_info = dot + dot + " checking paddle mobile results for {} -- {} ".format(
        green("【fusion】" if fuse else "【non fusion】"),
        green("【memory-optimization】" if mem_opt else "【non-memory-optimization】"))
    logs.append(optimization_info)
    pp_yellow(optimization_info)
    mobile_var_cache = {}
    for line in lines:
        parts = line.split(" ")
        if len(parts) < 2:
        ...
        if parts[1] == "load-time-cost":
            cost__format = "load time cost : {}".format(parts[2])
            logs.append(cost__format)
            pp_green(cost__format, 1)
        elif parts[1] == "predict-time-cost-avg":
            format = "predict time cost avg : {}".format(parts[2])
            logs.append(format)
            pp_green(format, 1)
        elif parts[1] == "predict-time-cost-max":
            max__format = "predict time cost max : {}".format(parts[2])
            logs.append(max__format)
            pp_green(max__format, 1)
        elif parts[1] == "predict-time-cost-min":
            min__format = "predict time cost min : {}".format(parts[2])
            logs.append(min__format)
            pp_green(min__format, 1)
        elif parts[1] == "preprocess-time-cost":
            time_cost__format = "preprocess time cost : {}".format(parts[2])
            logs.append(time_cost__format)
            pp_green(time_cost__format, 1)
        elif parts[1] == "var":
            var_name = parts[2]
            ...
            try:
                file_name = var_name.replace("/", "_")
                out_file = open(output_mobile_path + "/" + file_name, "w")
                for item in values:
                    out_file.write("{}\n".format(item))
                out_file.close()
            except:
...
def main():
    ...
    print("==================================================")
    print("")
    pp_yellow(dot + " start inspecting paddle mobile performance")
    ...
    push(mobile_src_root + "/test/build/test-net-performance")
